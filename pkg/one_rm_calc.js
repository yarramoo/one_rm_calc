...
*/
export function find_onerm(reps, weight) {
    try {
        const retptr = wasm.__wbindgen_add_to_stack_pointer(-16);
        wasm.find_onerm(retptr, reps, weight);
        var r0 = getInt32Memory0()[retptr / 4 + 0];
...
export class Tetris {
    ...
    */
    grid() {
        const ret = wasm.tetris_grid(this.__wbg_ptr);
        return ret >>> 0;
    }
}
...
async function __wbg_init(input) {
    ...
    if (typeof input === 'undefined') {
        input = new URL('one_rm_calc_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
